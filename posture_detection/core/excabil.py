...
class UserCalibrator:
    ...
    def __init__(self):
        ...
        self.min_samples_required = 10  # Minimum good samples needed
        self.good_samples_count = 0
        self.consecutive_bad_frames = 0
        self.max_consecutive_bad = 15 
    ...
    def add_calibration_frame(
        ...
        Returns:
        (success: bool, quality_score: float, message: str)
        """
        ...
        try:
            ...
            visible_count = len(landmark_points)
            visibility_ratio = visible_count / len(required_landmarks)
            
            # Check average confidence
            avg_confidence = 0.0
            if landmark_points:
                avg_confidence = sum(p.visibility for p in landmark_points.values()) / len(landmark_points)
            
            quality_score = (visibility_ratio * 0.5 + avg_confidence * 0.5) * 100
            
            # Determine if frame is good enough
            if visible_count < len(required_landmarks):
                self.consecutive_bad_frames += 1
                return (False, quality_score, f"Missing {len(required_landmarks) - visible_count} landmarks - Move closer or adjust position")
            
            if avg_confidence < 0.7:
                self.consecutive_bad_frames += 1
                return (False, quality_score, "Low confidence - Improve lighting or reduce motion")
            ...
            logger.debug(f"Good calibration sample added (total: {len(self.calibration_samples)})")
            return (True, quality_score, f"Good position! ({self.good_samples_count} samples)")
    ...
    def calibrate_flexibility(
        ...
        """
        Calibrate flexibility by measuring actual range of motion
        Stores min/max angles achieved during calibration
        """
        try:
            extractor = LandmarkExtractor()
        
            # Extract landmarks
            required_landmarks = self._get_flexibility_landmarks(exercise_name)
            landmark_points = extractor.extract_multiple_landmarks(
                landmarks, required_landmarks, image_width, image_height
            )
        
            if len(landmark_points) < len(required_landmarks):
                logger.warning("Not all landmarks visible for flexibility calibration")
                return False
        
        # Measure flexibility based on exercise type
            if exercise_name == 'squat':
                # Measure deepest squat
                hip = landmark_points['LEFT_HIP']
                knee = landmark_points['LEFT_KNEE']
                ankle = landmark_points['LEFT_ANKLE']

                from core.angle_calculator import calculate_knee_angle, calculate_hip_angle
                ...
                if knee_angle and hip_angle:
                    # Initialize flexibility profile if not exists
                    if not hasattr(self, 'flexibility_samples'):
                        self.flexibility_samples = {
                            'squat_knee': [],
                            'squat_hip': []
                        }
                
                # Collect samples
                self.flexibility_samples['squat_knee'].append(knee_angle)
                self.flexibility_samples['squat_hip'].append(hip_angle)
            ...
            # Measure shoulder range
                ...
                from core.angle_calculator import calculate_shoulder_angle
                shoulder_angle = calculate_shoulder_angle(hip, shoulder, elbow)
            
                if shoulder_angle:
                    if not hasattr(self, 'flexibility_samples'):
                        self.flexibility_samples = {'shoulder_flexion': []}
                    self.flexibility_samples['shoulder_flexion'].append(shoulder_angle)
        
            elif exercise_name == 'pushup':
                # Measure body alignment capability
                shoulder = landmark_points['LEFT_SHOULDER']
                hip = landmark_points['LEFT_HIP']
                ankle = landmark_points['LEFT_ANKLE']
            
                from core.angle_calculator import AngleCalculator
                alignment = AngleCalculator.calculate_body_alignment(shoulder, hip, ankle)
            
                if alignment:
                    if not hasattr(self, 'flexibility_samples'):
                        self.flexibility_samples = {'body_alignment': []}
                    self.flexibility_samples['body_alignment'].append(alignment)
        
            return True
        
        except Exception as e:
            logger.error(f"Error in flexibility calibration: {e}")
            return False

    def _get_flexibility_landmarks(self, exercise_name: str) -> list:
        """Get landmarks needed for flexibility measurement"""
        landmark_map = {
            'squat': ['LEFT_SHOULDER', 'LEFT_HIP', 'LEFT_KNEE', 'LEFT_ANKLE'],
            'shoulder_press': ['LEFT_SHOULDER', 'LEFT_ELBOW', 'LEFT_WRIST', 'LEFT_HIP'],
            'pushup': ['LEFT_SHOULDER', 'LEFT_HIP', 'LEFT_ANKLE'],
            'plank': ['LEFT_SHOULDER', 'LEFT_HIP', 'LEFT_ANKLE'],
            'bicep_curl': ['LEFT_SHOULDER', 'LEFT_ELBOW', 'LEFT_WRIST']
        }
        return landmark_map.get(exercise_name, ['LEFT_SHOULDER', 'LEFT_HIP'])

    def finalize_flexibility_calibration(self) -> bool:
        """
        Calculate final flexibility profile from collected samples
        Call this after flexibility calibration is complete
        """
        if not hasattr(self, 'flexibility_samples'):
            # No samples collected, use defaults
            self.flexibility_profile = FlexibilityProfile(
                squat_depth_angle=80.0,
                hip_flexion_range=120.0,
                shoulder_flexion_range=170.0,
                ankle_dorsiflexion=25.0
            )
        logger.info("Using default flexibility values")
        return True
    
        import numpy as np
    
        # Calculate min angles (deepest positions)
        squat_depth = np.min(self.flexibility_samples.get('squat_knee', [80.0]))
        hip_flexion = np.min(self.flexibility_samples.get('squat_hip', [120.0]))
        shoulder_range = np.max(self.flexibility_samples.get('shoulder_flexion', [170.0]))
    
        self.flexibility_profile = FlexibilityProfile(
            squat_depth_angle=float(squat_depth),
            hip_flexion_range=float(hip_flexion),
            shoulder_flexion_range=float(shoulder_range),
            ankle_dorsiflexion=25.0  # Still using default for this
        )
    
        logger.info(f"Flexibility profile created: squat_depth={squat_depth:.1f}°, "
                f"hip_flexion={hip_flexion:.1f}°, shoulder_range={shoulder_range:.1f}°")
